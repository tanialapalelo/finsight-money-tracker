...
export class OAuthClient<T> {
  ...
  // pass information to get token from oauth client
...
// set the url & user info from different oauth client
...
// additional security making sure the data is not from any malicious 3rd party
function createState(cookies: Pick<Cookies, "set">) {
  const state = crypto.randomBytes(64).toString("hex").normalize()
  cookies.set(STATE_COOKIE_KEY, state, {
    secure: true,
    httpOnly: true,
    sameSite: "lax", // lex allows discord to our server
